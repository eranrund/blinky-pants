...
unsigned char pattern = RemoteControl;
...
#include <pthread.h>
#include <arpa/inet.h>
#define CONTROL_PORT 9022

void * control_thread(void * arg) {
    unsigned char recv_buf[LED_COUNT * 3];
    int sock;
    ...
    myaddr.sin_port = htons(CONTROL_PORT);
    ...
    for (;;) {
        ...
        if ((recvlen == 4) && (recv_buf[0] == 'P') && (recv_buf[1] == 'A') && (recv_buf[2] == 'T')) {
            pattern = recv_buf[3];
            memset(leds, 0, sizeof(leds));
            printf("pattern: %d\n", pattern);
        }
        else if (recvlen == sizeof(recv_buf)) {
            for (i = 0; i < LED_COUNT; ++i) {
                leds[i].r = recv_buf[(i * 3) + 0];
                leds[i].g = recv_buf[(i * 3) + 1];
                leds[i].b = recv_buf[(i * 3) + 2];
            }
        }
    }
}
...
void LED_show() {
  int count;
        ...
        for (count = 0; count < LED_COUNT; ++count) {
            //write_color(
            write_gamma_color(
                &LED_buf.pixels[count],
                leds[count].r,
                leds[count].g,
                leds[count].b
            );
        }
        send_buffer(leds_fd,&LED_buf);  
}
...
void LED_setup() {
  ...
  if(lpd8806_init(&LED_buf,NUM_LEDS)<0) {
...
pthread_t control_thread_, beacon_thread_;
...
void setup()
{
    ...
    pthread_create(&control_thread_, NULL, control_thread, NULL);
    pthread_create(&beacon_thread_, NULL, beacon_thread, NULL);
    LED_setup();
...
//    if (pattern < 8) pattern = 13;
  ...
  if (loopCount == 0 && pattern != RemoteControl)
  {

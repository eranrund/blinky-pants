...
void warmWhiteShimmer()
{
  ...
  for (unsigned int i = 0; i < N_LEDS; i += 2)
  {
...
void colorExplosion()
{
  ...
  for (unsigned int i = 1; i < N_LEDS - 1; i++)
  {
...
void brightTwinkle()
{
      ...
      else
      {
        ...
        brightTwinkle_helper(1, 6, g_step > (unsigned int)(maxLoops - 100));
      }
...
int antipodal_index(unsigned int i) {
  int iN = i + TOP_INDEX;
...
int horizontal_index(unsigned int i) {
...
void Matrix_pat() {
  ...
  copy_led_array();
    for(unsigned int i = 1; i < N_LEDS; i++ ) {
